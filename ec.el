...
;; Last Change: Tue Feb 28 14:49:46 2006
...
;; ec.el is my first emacs lisp program written. This is a beta
...
;; * full mouse support (incl. wheelmouse)
...
;; When the users presses 'ret' on a file, the default (first) action
;; will be executed. This can be used e.g. to run a pdf viewer when on a
;; pdf-file. 
(defvar ec-actions 
  '(("\.dvi$" 
     [view "view" "xdvi %f"])
...
(defvar ec-get-gid-function 'getent 
  "Method used to lookup the group of a file. This is impossible in
emacs, therefore an external program has to be used. Supported methods
are currently `getent' (linux, solaris) and `nicl' (Mac OS X).")
...
(defvar ec-group-id-alist nil
  "alist that holds already read group ids and the corresponding name")
...
;; use ctrl-t to toggle between mode line formats
(defvar ec-mode-line-mtime-format 
  (list '(12 . ec-mode-line-mtime)
 	" "
 	'(10 . ec-mode-line-perm)
 	" "
 	'ec-mode-line-size)
  "Format of the mode line: mtime perm, size")
(defvar ec-mode-line-uid-format 
  (list '(8 . ec-mode-line-uid)
	":"
	'(8 . ec-mode-line-gid)
	"  "
	'(10 . ec-mode-line-perm)
	" "
	'ec-mode-line-size)
  "Format of the mode line: uid:gid perm, size")
(defvar ec-buffer-side nil)
(defvar ec-cursor-overlay nil 
  "Cursor'bar'")
...
(defvar ec-original-frame-title-format nil
  "Used to reset the original value of frame-title-format")
...
(defvar ec-cursor-face 'ec-cursor-face
  "Used for highlighting the current line")



;;; define some faces
(make-face ec-cursor-face)
(set-face-foreground 'ec-cursor-face "white")
(set-face-background 'ec-cursor-face "blue")

;;; define some categories for displaying files dirs, symlinks etc.
(put 'ec-cat-cursor 'face 'ec-cursor-face) ; cursor `bar'
...
(make-variable-buffer-local 'ec-mode-line-uid)
(make-variable-buffer-local 'ec-mode-line-gid)
...
(substitute-key-definition ;; this is crazy!!!
 'self-insert-command 
 'ec-isearch ec-mode-map global-map)

;; mode line
;; iBook with Mac OS X, strange keyboard

(define-key ec-mode-map (kbd "TAB") 'ec-other-window)
(define-key ec-mode-map (kbd "RET") 'ec-execute-file)

(define-key ec-mode-map [?+] 'ec-select-unselect)
;; pc-keyboard
(define-key ec-mode-map [?\C-t]     'ec-toggle-modeline)
...
(define-key ec-mode-map "\C-o" 'ec-same-directory-other-window)

;; mouse movement (incl. wheel mouse btn. 4,5)
...
;; 				     (interactive)
;; 				     (ec-mouse-scroll-down-some-files -1)))
(define-key ec-mode-map [wheel-down] '(lambda ()
					(interactive)
					(ec-mouse-scroll-down-some-files 1)))
(define-key ec-mode-map [wheel-up] '(lambda ()
				      (interactive)
				      (ec-mouse-scroll-down-some-files -1)))
...
(defun ec ()
  ...
  (setq ec-original-frame-title-format  frame-title-format)
...
ec-buffer-side to SIDE. Returns pointer to created buffer."
  ...
  (setq mode-line-format ec-mode-line-mtime-format)
...
(defun ec-window-configuration-hook ()
  ...
  (when (or (equal (buffer-name) "*ec-left*")
...
(defun ec-quit ()
  "Quit ec, restore scrollbar etc."
  (interactive)
  (remove-hook 
   'window-configuration-change-hook 
   'ec-window-configuration-hook)
  (setq frame-title-format ec-original-frame-title-format)
  (modify-frame-parameters (selected-frame) 
			   (list (cons 'vertical-scroll-bars ec-original-scroll-bar))) 
  (when (buffer-live-p ec-buffer-left)
    (kill-buffer ec-buffer-left))
  (when (buffer-live-p ec-buffer-right)
...
(defun ec-copy-selected-files ()
  ...
  (interactive)
  ;; think about what to do when copying a symlink. 
  ;; mc has (at least) three options:
  ;; 1) nothing -> symlinks will be broken (if referencing file is not
  ;;    been copied)
  ;; 2) follow links -> the referencing file will be copied, not the link
  ;; 3) Stable Symlinks -> the symlinks will point to the original
  ;;    file, i.e.  ~/symtest/foo and ~/symtest/symlink -> foo will be
  ;;    copied to ~/test/foo and ~/test/symlink -> ../symtest/foo
  ;; the current implementation follows strategy 2 (0.96b4)

  ;; XXX the if clause should be done as a cond with a 'this cannot
  ;;    happen' fallback 
  ;; 5 cases:
  ;; 1: nothing selected, cursor on file or one file selected 
  ;;    "copy file to" 
  ;; 2: nothing selected, cursor on dir or one dir selected 
  ;;    "copy dirto" 
  ;; 3: only dirs selected: "copy (num) selected dirs to" 
  ;; 4: only files selected: "copy (num) selected files to:" 
  ;; 5: some dirs and some files selected 
  ;;    "copy (num) sel. files/directories to:" 
	...
	(if (ec-fileorsymlink-directory-p thisfile)
	    ...
	    (progn 
	      (setq dest (read-string 
			  ...
			  (concat (ec-dir-other-buffer) thisfile) 'ec-history))
...
(defun ec-delete-directories-recursively (dirs &optional dontask)
  "Delete directories recursively. Delete all dirs listed in DIRS. If
DONTASK is non-nil, user confirmation is required."
  (mapcar '(lambda (dir)
	     (message "recursive deleting %s" dir)
	     (if (ec-fileorsymlink-directory-p dir)

		 ;; we have a directory:
		 ;; if all dontask and directory empty and y-or-no-p
		 ;; do not eval to nil, delete dir
		 (when (or dontask     ;put dontask first for efficiency
			   (not (directory-files 
				 dir t
				 ec-directory-without-dots-regexp))
			   (y-or-n-p 
			    (format 
			     "Directory %s not empty. Delete it recursively? "
			     dir )))
		   (ec-delete-directories-recursively 
		    (directory-files dir t 
				     ec-directory-without-dots-regexp) t)
		   (delete-directory dir))
	       ;; file
	       (ec-delete-files (list dir))))
	  dirs))
...
(defun ec-delete-selected-files ()
  ...
  ;; case 3:
  (when (and (= (length ec-selected-files) 0)
	     (> (length ec-selected-dirs) 0))
    (let ((num (length ec-selected-dirs)))
      (when (y-or-n-p (format "Delete %d selected dirs? " num))
	(ec-delete-directories-recursively 
	 (ec-num-to-files (ec-first ec-selected-dirs))))))
...
(defun ec-make-directory (dir)
  ...
  (make-directory dir)
  (ec-reread-directory)
  (ec-goto-fileno (ec-whereis-filename  dir) t))
...
(defun ec-deselect-files-query (pattern)
  ...
  (let ((i 0)
    ...
    (while (< i max)
      ...
      (when (and (not (ec-file-directory-p i))
...
selected ones will be unselected. Works only for files right now"
  ...
  (save-excursion
    (let ((i 0)
	  (max ec-number-of-files-and-dirs ))
      (while (< i max)
	(when (not (ec-file-directory-p  i))
...
(defun ec-same-directory-other-window ()
  "Show the same directory in the other window."
  (interactive)
  (let ((thisdir default-directory))
    (save-excursion
      (set-buffer ec-other-buffer)
      (ec-goto-directory thisdir)))
  (ec-show-cursor))

;;; reading dirs and stuff like that
(defun ec-up-directory ()
  "Go up one directory. Move the cursor to the old directory."
  (interactive)
  (ec-delete-isearch-string)
...
(defun ec-toggle-modeline ()
  "Toggle between 
uid:gid perm size
mtime   perm size"
  (interactive)
  (setq mode-line-format
	(cond ( (eq mode-line-format ec-mode-line-mtime-format)
		ec-mode-line-uid-format)
	      ( (eq mode-line-format ec-mode-line-uid-format)
		ec-mode-line-mtime-format)
	      ( t 
...
ec-mode-line-xxx." 
  (let ((attribs (nth 2 (aref ec-directoryv fileno)))) ; file-attributes
    ...
    (setq ec-mode-line-gid  (ec-group-name (nth 3 attribs)))
...
(defun ec-context-menu (&rest)
  ...
  (let* ((thisfile (ec-file-at-point))
	 ...
	 (lastactions (reverse (cdr (ec-default-action thisfile))))
...
(defun ec-previous-line () 
  "Move cursor one line up and show nice cursor."
  (interactive)
  (ec-delete-isearch-string)
  (forward-line -1)
  (ec-show-cursor (point)))
...
(defun ec-num-to-files (arg)
  ...
  ;; isn't this stupid what I am doing here? XXX
  (let ((mylist))
    (mapcar '(lambda (x)
	       (setq mylist 
		     (append mylist 
...
(defun ec-restore-buffers (&optional buf)
  "Show the two ec buffers horizontally and delete buf if not nil"
  (interactive)
  (ec-delete-isearch-string)
...
(defun ec-reread-directory ()
  "Reread and redisplay current directory. Should keep selected files selected. (Todo)"
  (interactive)
  (ec-delete-isearch-string)
  (setq ec-window-configuration (current-window-configuration))
...
(defun ec-compare-filenames (filename1 filename2)
  "Case insensitive comparison of two filenames (strings)."
  (let ((tmp (compare-strings filename1  0 nil filename2 0 nil t)))
    (or (eq tmp t) ; tmp can be <0 t or >0
	( > 0 tmp))))

;; (defun ec-compare-filenames (filename1 filename2)
;;   "Case insensitive comparison of two filenames (strings)."
;;   ;; there must be an easier way for that
;;   (or (string= filename1 filename2)
;;       ( > 0 (compare-strings filename1  0 nil filename2 0 nil t))))
...
(defun ec-read-directory ()
 ...
 display it after deleting content of buffer."
  ...
  (let* ((dirlist (ec-file-lists default-directory))
	 (files (sort (car (cdr dirlist)) 'ec-compare-filenames))
	 (dirs (sort (car dirlist) 'ec-compare-filenames))
	 (overlay nil)
	 (beg)
	 (end)
	 (i 0)
    ...
    ;; display the current directory in the header-line
    ...
    (while dirs
      ...
      ;; put in vector
      ;; 0: name, 1: overlay, 2: file-attributes, 3: point, 4: dir, 
      ;; 5: file-or-symlink
    ...
    (while (< i  (length ec-directoryv))
      ;; please tell me why ec-fileorsymlink-directory-p!!
      (if (ec-fileorsymlink-directory-p i)
...
(defun ec-view-file ()
  ...
  (let* ((filename (ec-file-at-point))
    ...
    (when (not (file-directory-p full-filename))
...
(defun ec-delete-cursor ()
  "Delete cursor in current window."
  (delete-overlay ec-cursor-overlay))


;;; movement
(defun ec-scroll-down-some-files (&optional ratio dir)
  "Set cursor some files closer to the beginning/end. DIR is the direction, negative means cursor towards the first file, postive or nil means cursor closer to the end. If the RATIO is x, you have to invoke this command x times to move to the end (beginning) of the file list, if you are at th beginning (end). E.g. if RATIO is 2, the first invocation goes to the middle of the list, the second to the end (beginning)."
  (interactive) 
  (ec-delete-isearch-string)
...
;;;; gid support
(defun ec-group-name (gid)
  "Like user-login-name, just with group id"
  ;; warning: this will break! 
  ;; 1) on remote systems
  ;; 2) on systems with no getent function
  ;; 3) on systems with no 'group' 

  ;; ec-group-id-alist is nil at the beginning. If the the gid is in
  ;; the alist, just return the assoc. If the gid is  not in the
  ;; alist, look it up and store the result in the alist. This means,
  ;; that once we looked up the value, it must not change. (At least
  ;; the change will not be displayed.)
  (let ((groupname (cdr (assoc gid ec-group-id-alist))))
    (if groupname groupname
      (with-temp-buffer
	;;(message "calling getent")
	(cond ((eq ec-get-gid-function 'getent)
	       (if (= 2 (call-process "getent" nil t nil "group" 
				      (number-to-string gid)))
		   ;; exit status 2 means 'no such group' 
		   ;; is that so? --pg 16.10.2001
		   (setq groupname "*inv*") ;invalid group id
		 (goto-char (point-min))
		 (setq groupname 
		       (buffer-substring 
			(point-min) 
			(1- (re-search-forward ":" (point-max)
					       t))))))
	      ((eq ec-get-gid-function 'nicl)
	       (call-process "/usr/bin/nicl" nil t nil "." "-read" 
			     (concat "/groups/gid=" (number-to-string gid)) 
			     "name")
	       (setq groupname
		     (progn 
		       (goto-char (point-min))
			...
			(- (re-search-forward "\n" (point-max) t) 1)))))
	      (t (setq groupname "*inv*")))
	;; put the result in the alist... strange syntax...
	(setq ec-group-id-alist 
	      (append `(,(cons gid groupname)) ec-group-id-alist ))
	groupname))))

;; question: why did I use ec-fileorsymlink-directory-p??? DOCUMENT IT!
